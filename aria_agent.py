...
<style>
:root{
  --bg1:#111827; --bg2:#0b1020; --card: rgba(18,24,38,.95);
  --white:#fff; --muted:#dfe7ff; --soft:#c7cdd8;
  --blue:#3b82f6; --violet:#8b5cf6; --green:#10b981; --yellow:#FFD700; --danger:#ef4444;
}
.main{
...
.analysis-card,.metric-card,.thought-bubble,.glass,.neon-card{
  background: var(--card); backdrop-filter: blur(2px); -webkit-backdrop-filter: blur(2px);
  border:1px solid rgba(255,255,255,.18); border-radius:18px; padding:20px;
  box-shadow:0 12px 40px rgba(0,0,0,.35);
}
/* Titres très visibles */
h1,h2,h3,h4,h5,h6{ color: var(--white) !important; }
.subtitle-white{ color:var(--white)!important; font-weight:800!important; letter-spacing:.3px; text-shadow:0 0 6px rgba(0,0,0,.55); }
.subtitle-yellow{ color:var(--yellow)!important; font-weight:800!important; letter-spacing:.3px; text-shadow:0 0 10px rgba(255,215,0,.75); }
.muted{ color:var(--soft)!important; }
/* Boutons */
.stButton > button{
  background:linear-gradient(135deg,var(--blue),var(--violet)); color:#fff; border:0; border-radius:12px; font-weight:700;
  box-shadow:0 10px 25px rgba(59,130,246,.35); transition: transform .2s, box-shadow .2s;
}
.stButton > button:hover{ transform: translateY(-2px); box-shadow:0 16px 35px rgba(139,92,246,.45); }
/* Selectbox */
.stSelectbox > div > div{
  background: rgba(255,255,255,.06)!important; color:#fff!important; border:1px solid rgba(255,255,255,.25)!important; border-radius:10px!important;
}
/* Pensées */
.thought-bubble{ border-left:5px solid var(--blue); animation: slideIn .45s ease-out; }
@keyframes slideIn{ from{opacity:0; transform:translateX(-18px);} to{opacity:1; transform:translateX(0);} }
/* Badge “Simulation Active” clignotant */
.sim-badge{
  display:inline-flex; align-items:center; gap:.5rem; background: rgba(255,215,0,.12);
  border:1px solid rgba(255,215,0,.6); padding:.35rem .75rem; border-radius:999px;
...
@keyframes neonBlink{ 0%,100%{ filter:drop-shadow(0 0 8px rgba(255,215,0,.9)); opacity:1;} 50%{ filter:drop-shadow(0 0 2px rgba(255,215,0,.3)); opacity:.65;} }
/* Avatar agent */
.agent-avatar{
  ...
  position:relative; box-shadow:0 0 24px rgba(59,130,246,.6); animation: slowSpin 10s linear infinite;
}
.agent-avatar::after{
...
class ARIAAgent:
    ...
    # --- Plotly Gauges (OK même si PLOTLY_OK=False : on ne l'appellera pas) ---
    def generate_confidence_gauge(self):
        ...
        fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                          font={'color':"white",'family':"Inter, Arial"}, height=320)
        return fig

    def generate_neural_network_viz(self):
        if not PLOTLY_OK:
            return None
        # Construction robuste (aucune dépendance externe)
        n_nodes = 20
...
def main():
    ...
    with left:
        ...
        status_color = {
            "idle": "#6b7280", "thinking": "#f59e0b", "analyzing": "#3b82f6", "completed": "#10b981"
        ...
        <div style='text-align:center; margin:8px 0 14px 0;'>
            ...
            <div style="margin-top:8px; display:flex; align-items:center; gap:8px; justify-content:center;">
                <span style="width:14px;height:14px;border-radius:50%;border:2px solid rgba(255,255,255,.85);background:{status_color}; box-shadow:0 0 12px {status_color};"></span>
                ...
                for idx, rec in enumerate(recs, 1):
                    ...
                    <div class="neon-card" style="background:rgba(139,92,246,.12); border-radius:14px; padding:14px; margin:10px 0;">
                        <div style="display:flex; gap:12px; align-items:flex-start;">
                            <div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg, var(--blue), var(--violet)); display:flex;align-items:center;justify-content:center;color:white;font-weight:800;">{idx}</div>
        ...
        with st.sidebar:
            ...
            if PLOTLY_OK:
                try:
                    ...
                    st.info("Le graphe neural a été désactivé suite à une erreur. (Plotly OK mais figure indisponible)")
            else:
